...
// Fetch uId dynamically based on userName
    ...
    ClockModel data = ClockModel(
      ...
      uId: fetchedUid, // Dynamically fetched
  ...
  Future<void> fetchClockLogs() async {
    ...
    if (snapshot.docs.isNotEmpty) {
      clockInTime.value = ClockModel.fromMap(snapshot.docs.first.data(), snapshot.docs.first.id).clockInTime;
    } else {
  ...
  void listenClockDataUpdates(BuildContext context, String id) {
    ...
    try {
      ...
      clockDataStream = firebaseFirestore
      ...
      clockDataStream.listen((documentSnapshots) {
        ...
        if (documentSnapshots.exists) {
          ...
          clockInTime.value = (documentSnapshots['clockInTime'] as Timestamp?)?.toDate();



          clockOutTime.value = (documentSnapshots['clockOutTime'] as Timestamp?)?.toDate();
